...
- `GROUP_INCLUDES` (top-level const in `template.html`) records group→group subset relationships for filtering purposes only: when a key group is selected in the group filter, events tagged with any of its listed groups are shown too, since that group's roster is a subset of the listed one's (currently `ドイツ組` ⊂ `山登り`, since だいちゃん/そうし are both in 山登り's roster — see `GROUP_MEMBERS` in `generate.js`). This does **not** change what badge an event displays (still whatever `グループ` tags it actually has) — it only widens which tags count as a match while the `ドイツ組` filter button is active. The button's own `.on` highlight still reflects `selGroups` literally (`syncFilterButtons()` is unaffected). If `GROUP_MEMBERS` rosters change such that the subset relationship no longer holds, update `GROUP_INCLUDES` to match.
