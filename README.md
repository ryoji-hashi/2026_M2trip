...
`template.html` を開き、`TRIP_NAME = "グループ旅行 2026"` を書き換えてください。

### 参加メンバー・グループの管理

予定ごとの参加者は、Notionの「参加メンバー」(マルチセレクト)で指定します。誰がその予定にいるかをそのまま選ぶだけで、Webページのバッジ表示・人数集計・メンバー別フィルターすべてに自動反映されます（コード側に「Aチーム」のような固定グループを追加・修正する必要はありません）。

- **全員参加の判定**: 選んだメンバー数が旅行全体の人数と一致すると自動的に「🌐 全員」バッジになります。
- **チーム名で表示したい場合**: 「チーム表示名」(テキスト)に好きなラベル（例:「Aチーム」「観光班」）を入れると、メンバー名の代わりにそのラベルがバッジに表示されます。空欄ならメンバー名（5人以上は人数表示）が自動表示されます。
- **旅行全体のメンバー名簿を変更する場合のみ**、`generate.js` の `ALL_MEMBERS` 配列を編集してください（途中離脱・合流ではなく、参加者そのものが増減したとき）。
